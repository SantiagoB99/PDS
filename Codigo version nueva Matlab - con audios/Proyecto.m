...
function [cuerda, frecuencia] = analizar(filtered_signals, Fs)
    ...
    for i = 1:size(filtered_signals, 2)
        ...
        % fprintf('La frecuencia con la amplitud mï¿½xima de la seï¿½al de salida filtrada para el filtro %i es %.2f Hz y su amplitud es %f\n', i, freq_max_amplitude, max_amplitude);
    end
    ...
    fprintf('La cuerda que se estï¿½ tocando es la %i\n', cuerda)
    ...
    for i = 1:6
        ...
        if cuerda == i 
            ...
            if frecuencia > frec_conocida*0.98 && frecuencia < frec_conocida*1.02
                fprintf('La cuerda %i estï¿½ afinada\n', cuerda);
            else
                fprintf('La cuerda %i no estï¿½ afinada\n', cuerda);
